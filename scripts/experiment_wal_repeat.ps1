<#
  experiment_wal_repeat.ps1 — Lặp lại phép đo baseline và WAL để kiểm tra tính tái lập.
...
$results = @()
foreach ($lap in 1..3) {
  foreach ($mode in @("baseline", "wal")) {
    $name = "6_${mode}_lap$lap"
    Write-Host ""
    Write-Host "=== $name ==="
    ...
    if ($mode -eq "wal") { Db "set-wal" | Out-Null } else { Db "set-delete" | Out-Null }
    $modeNow = Db "journal-mode"
    Write-Host "  journal_mode truoc khi khoi dong server = $modeNow"
    if ($mode -eq "wal" -and $modeNow -ne "wal") { throw "Khong bat duoc WAL" }
    if ($mode -eq "baseline" -and $modeNow -ne "delete") { throw "Khong ep duoc ve DELETE" }

    & powershell -ExecutionPolicy Bypass -File (Join-Path $Repo "scripts\reset_sut.ps1") | Out-Null
    Db "seed-orders" "$SeedOrders" | Out-Null

    # Bang chung truc tiep WAL dang hoat dong: SQLite tao file sidecar -wal
    $walFile = Join-Path $SutBackend "database.sqlite-wal"
    Write-Host "  file database.sqlite-wal ton tai: $(Test-Path $walFile)"
